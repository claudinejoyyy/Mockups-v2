      ...
      if(req.session.email && req.session.sino == 'pharmacist'){
        if(req.session.sino == 'pharmacist') {
          if (data.sub == 'confirm') {
            ...
            db.query(confirmPrescriptionSQL + 'INSERT into activity_logs(account_id, time, type, remarks) VALUES ('+Aid+',"'+moment(new Date()).format('MMM/DDD/YYYY HH:mm')+'", "approvedPrescription", "Approved prescription for: '+req.query.prescriptionPatientName+'");', function(err){
              if(err){
                console.log(err);
              } else {
            ...
            db.query(cancelPrescriptionSQL + 'INSERT into activity_logs(account_id, time, type, remarks) VALUES ('+Aid+',"'+moment(new Date()).format('MMM/DDD/YYYY HH:mm')+'", "cancelPrescription", "Cancelled prescription for: '+req.query.prescriptionPatientName+'");', function(err){
              if(err){
                console.log(err);
              } else {
